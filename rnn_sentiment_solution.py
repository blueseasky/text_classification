...
parser.add_argument('-e', '--epoch', help='Number of the training epoch.', type=int, default=100)
...
with open(mr_positive_filename, 'r',encoding='utf-8') as fin:
    lines = fin.readlines()
    logger.info('Positive Instances: %d' % len(lines))
    mr_txt.extend(lines)
    mr_label.extend([1] * len(lines))
with open(mr_negative_filename, 'r',encoding='utf-8') as fin:
    lines = fin.readlines()
...
for i, sent in enumerate(mr_txt):
    ...
    if(l< max_len-2):
         mr_insts[i,l+1:,:] = word_embedding.wordvec("</s>")
...
if args.model == "rnn":
    ...
    print('embedding_dim',word_embedding.embedding_dim())
