...
from keras.layers import Dense, LSTM, GRU, SimpleRNN, GlobalAveragePooling1D, Activation, Input
...
parser.add_argument('-s', '--size', help='The size of each batch used to be trained.', type=int, default=90)
parser.add_argument('-l', '--rate', help='Learning rate of AdaGrad.', type=float, default=0.02)
parser.add_argument('-e', '--epoch', help='Number of the training epoch.', type=int, default=40)
parser.add_argument('-m', '--model', help="Which model to use", type=str, default="lstm_attention")
...
with io.open(mr_positive_filename, 'r', errors='ignore') as fin:
    lines = fin.readlines()
...
seq_length = train_insts.shape[1]
...
start_time = time.time()
model.fit(train_insts, train_labels, validation_data=(test_insts,test_labels),epochs=epoch, batch_size=batch_size, verbose=2)
end_time = time.time()
